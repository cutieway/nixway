...
2. Preserve the ownership boundary: hardware in `hosts/`, system behavior in `modules/nixos/`, user behavior in `modules/home/` or `home/lexi/`. Module selection (NixOS imports and home-manager sharedModules) goes in `hosts/<hostname>/default.nix`.
